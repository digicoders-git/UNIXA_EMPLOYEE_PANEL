...
const AssetsHistory = () => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const { data } = await api.get(`/employee-assets/history?${params.toString()}`);
    ...
    <VStack spacing={8} align="stretch" w="full" pb={10}>
      <HStack justify="space-between" align="center">
        <VStack align="start" spacing={1}>
          <Heading size="lg" fontWeight="900" color="slate.800"><Icon as={FaHistory} mr={2} color="purple.600" />Assets History</Heading>
          <Text color="slate.500" fontSize="sm">Track all asset assignments and returns</Text>
        </VStack>
        <HStack spacing={3}>
          <HStack>
      ...
      <Box bg="white" borderRadius="2xl" border="1px solid" borderColor="slate.100" overflow="hidden" boxShadow="sm">
        {history.length === 0 ? (
          <Box p={10} textAlign="center">
            <Text color="slate.500">No history found</Text>
          </Box>
        ) : (
          <Table variant="simple">
            ...
            <Tbody>
              {history.map((item, idx) => (
                <Tr key={idx} _hover={{ bg: "slate.50" }}>
                  <Td>
                    <HStack spacing={2}>
                      <Icon as={assetIcons[item.assetType] || FaBox} color="brand.500" />
                      <Box>
                        <Text fontWeight="bold">{item.assetName}</Text>
                        <Text fontSize="xs" color="slate.500">ID: {item.assetId}</Text>
                      </Box>
                    </HStack>
                  </Td>
                  ...
                  <Td><Text fontSize="sm" color="slate.600">{item.remarks || "-"}</Text></Td>
                </Tr>
              ))}
            </Tbody>
          </Table>
